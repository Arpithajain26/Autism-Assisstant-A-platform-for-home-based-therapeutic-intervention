...
// ── Health ────────────────────────────────────────────────────────────────────
app.get("/", (_req, res) =>
  res.json({ status: "ok", message: "🚀 Autism Assistant API running" }),
);
...
app.post("/api/auth/register", (req, res) => {
  const { role, name, email, password, age, parentEmail, specialization } =
    req.body;
...
app.post("/api/assessment/submit", (req, res) => {
  ...
  if (!childId || !Array.isArray(answers)) {
    return res
      .status(400)
      .json({ error: "childId and answers array required." });
  }
  ...
  const children = childIds
    ...
    .map((c) => ({
      ...
      assignedActivities: (c.assignedTasks || [])
        .map((tid) => activities.find((a) => a._id === tid))
        .filter(Boolean),
...
app.get("/api/child/:childId/tasks", (req, res) => {
  ...
  const assigned = (child.assignedTasks || [])
    .map((id) => activities.find((a) => a._id === id))
    .filter(Boolean);
